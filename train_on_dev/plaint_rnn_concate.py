...
class Hierachi_RNN(object):
    ...
    def model_constructor(self, wemb_size = None):
        ...
        self.encoding_layer()
        story1 = T.concatenate([self.train_encodinglayer_vecs[0], self.train_encodinglayer_vecs[1]], axis = 1)
        story2 = T.concatenate([self.train_encodinglayer_vecs[0], self.train_encodinglayer_vecs[2]], axis = 1)
    ...
    def eva_func(self, val_or_test):
        ...
        if self.loss_type == "hinge":
            ...
            if val_or_test == 'val' and self.best_val_accuracy <= acc:
                ...
                self.record_flag = False
                with open('./plaint_concate'+self.saving_path_suffix+'.pkl','w') as f:
