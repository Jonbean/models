...
import LSTM_sequence_Decoder as LSTM_Decoder
...
class Hierachi_RNN(object):
    def __init__(self, args):
        ...
        self.discrim_lr = float(args.discrit_lr)
        self.generat_lr = float(args.generat_lr)

        self.mode = args.rnn_output_mode
        self.GRAD_CLIP = 10.0
        if args.nonlin_func == 'default':
            self.nonlin_func = lasagne.nonlinearities.tanh
        else:
            self.nonlin_func = None

        self.bias = 0.001
        self.dnn_generator_settings = map(int, args.generator_setting.split('x'))
        self.dnn_discriminator_setting = map(int, args.score_func_setting.split('x'))
        self.decoder_units = map(int, args.decoder_units.split('x'))

        self.reasoning_type = args.reasoning_type

        self.loss_type = args.loss_type
        self.score_func = args.score_func
        self.GAN_type = args.GAN_obj
        self.discrim_regularization_level = args.drl 
        self.generat_regularization_level = args.grl 
        # self.random_input_type = random_input_type
        if args.saving == "local":
        ...
        self.regularization_index = float(args.reg_factor)
    ...
    def decoding_layer(self, input_variable, input_mask):
        ...
        return predict_seq, batchsize, max_len
    ...
    def model_constructor(self, wemb_size = None):
        ...
        for i in range(self.story_nsent+2):
            ...
            self.inputs_masks.append(T.matrix('story'+str(i)+'_mask', dtype='int64'))
        ...
        self.fake_endings = lasagne.layers.get_output(self.DNN_generator.output, {self.DNN_generator.l_in: self.plot_rep})
        ...
        for i in range(self.story_nsent+1):
            prediction, batch_size, max_len = self.decoding_layer(self.train_encodinglayer_vecs[i], self.inputs_masks[i])
            cost = lasagne.objectives.categorical_crossentropy(prediction, T.flatten(self.inputs_variables[i]) - 1)
            cost_reshape = cost.reshape((batch_size, max_len, 1))
            cost_after_mask = cost_reshape * self.inputs_masks[i]
            self.decoding_cost_ls.append(cost_after_mask)
...
#            self.max_score_index = T.argmax(all_other_score, axis = 1)
        ...
        if self.loss_type == 'hinge':
            ...
            self.discrim_score_vec = T.gt(self.discrim_score, 0.0) * self.discrim_score
            ...
            self.generat_score_cost = - lasagne.objectives.aggregate(self.score2, mode = 'mean')
        else:
            prob1 = lasagne.nonlinearities.softmax(self.score1)
            prob2 = lasagne.nonlinearities.softmax(self.score2)

            self.discrim_cost = lasagne.objectives.categorical_crossentropy(prob1, T.ones((self.batchsize, )).astype('int64'))
            self.generat_score_cost = lasagne.objectives.categorical_crossentropy(prob2, T.zeros((self.batchsize, )).astype('int64'))


        if self.batchsize >= 300 and self.GAN_type == 'JSD':
        ...
        elif self.GAN_type == 'MidD':
            self.generat_cost = T.sqrt(T.sum(T.sqr(self.ending1_rep - self.fake_endings))) 
        elif self.GAN_type == 'ComD':
            self.generat_cost = T.sqrt(T.sum(T.sqr(self.ending1_rep - self.fake_endings))) - self.generat_score_cost 
        elif self.GAN_type == 'ComW':
            self.generat_cost = T.sqrt(T.sum(T.sqr(self.fake_mean - self.real_mean))) - self.generat_score_cost
        elif self.GAN_type == 'W':
            self.generat_cost = -self.generat_score_cost
        else:
            self.generat_cost = T.sqrt(T.sum(T.sqr(self.fake_mean - self.real_mean)))
...
#        fake_ending_interpretation = self.decoding_layer(self.fake_endings, T.ones((self.batchsize, 20)))
        ...
        all_decoder_updates = lasagne.updates.adam(self.decoding_cost, self.decoder.all_params)
        ...
        all_discrim_updates.update(all_decoder_updates)  
        if self.loss_type == 'hinge':        
            self.train_func = theano.function(self.inputs_variables[:5] + self.inputs_masks[:5], 
                                             [self.discrim_cost, self.generat_cost, 
                                             self.score1, self.score2, self.train_encodinglayer_vecs[4],self.fake_endings, self.generat_score_cost],
                                             updates = all_discrim_updates)
        else:
            self.train_func = theano.function(self.inputs_variables[:5] + self.inputs_masks[:5], 
                                             [self.discrim_cost, self.generat_cost, self.score1, 
                                              self.score2, self.train_encodinglayer_vecs[4],self.fake_endings, self.generat_score_cost],
                                             updates = all_discrim_updates)
    ...
    def begin_train(self):
        ...
        for epoch in range(N_EPOCHS):
            ...
            for batch in range(max_batch):
                ...
                cost_through_score_func = result_list[6]
                ...
                total_gene_score_cost += cost_through_score_func
#                total_decoding_cost += decoding_cost
                ...
                if batch % test_threshold == 0 and batch != 0:
                    ...
                    print "cost through score function:", total_gene_score_cost/(batch+1)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--generat_lr", type=float, default = 0.001, help="generator's learning rate")
    ...
    parser.add_argument("--decoder_units", type=str, default = '301x600', help="RNN decoder units setting")
    parser.add_argument("--generator_setting", type=str, default = '300x600x300', help="DNN generator hidden layers and units")
    parser.add_argument("--drl", type=int, default = 1, help="discrit DNN regularization level")
    parser.add_argument("--grl", type=int, default = 1, help="generat DNN regularization level")
    parser.add_argument("--GAN_obj", type=str, default='MidW', choices=['JSD', 'MidW', 'ComW', 'MidM', 'ComM', 'W'], help="the objective function of GAN to optimize")
