...
#            self.max_score_index = T.argmax(all_other_score, axis = 1)
        ...
        if self.batchsize >= 300:
            self.feature_diff_cost = T.nlinalg.trace(T.dot(real_cov_inverse, self.fake_covariance) + T.dot(fake_cov_inverse, self.real_covariance)) + \
                                T.dot(T.dot((self.fake_mean - self.real_mean).T, (fake_cov_inverse + real_cov_inverse)), (self.fake_mean - self.real_mean))
            self.score_min_cost = -T.sum(self.score2)

        else:
            self.feature_diff_cost = T.sqrt(T.sum(T.sqr(self.fake_mean - self.real_mean))) 
            self.score_min_cost = - T.sum(self.score2)
    ...
    def begin_train(self):
        ...
        for epoch in range(N_EPOCHS):
            ...
            for batch in range(max_batch):
                ...
                result_list = []
                if best_train_acc > 90.0:
                    result_list = self.score_min_train_func(train_story_matrices[0],
                    ...
                    result_list = self.feature_diff_train_func(train_story_matrices[0],
                                                  train_story_matrices[1],
                                                  train_story_matrices[2],
                                                  train_story_matrices[3],
                                                  train_end_matrix,
