...
class Hierachi_RNN(object):
    ...
    def model_constructor(self, wemb_size = None):
        ...
        self.merge_ls1 = [T.reshape(tensor, (tensor.shape[0], 1, tensor.shape[1])) for tensor in self.train_encodinglayer_vecs[:self.story_nsent]]
        ...
        original_storyNoend = reasoner_result1

        # self.alternative_end.shape = self.DNN_liar.output
        self.alternative_end = lasagne.layers.get_output(self.DNN_liar.output, {self.DNN_liar.l_in: original_storyNoend})
