...
class BGRUEncoder(object):
    def __init__(self, LAYER_1_UNITS, dropout_rate = 0.0):
    ...
    def build_model(self, WordEmbedding_Init = None):
        ...
        # The LSTM layer should have the same mask input in order to avoid padding entries
        l_grurnn = lasagne.layers.recurrent.GRULayer(l_drop, num_units=self.layer1_units, backwards=False,
                                                    learn_init=True, 
                                                    gradient_steps=-1, grad_clipping=self.GRAD_CLIP, 
                                                    precompute_input=True, mask_input=self.l_mask
                                                    )
        l_grurnn_back = lasagne.layers.recurrent.GRULayer(l_drop, num_units=self.layer1_units, backwards=True,
                                                    learn_init=True, 
                                                    gradient_steps=-1, grad_clipping=self.GRAD_CLIP, 
                                                    precompute_input=True, mask_input=self.l_mask
                                                    )
