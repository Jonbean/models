...
class Hierachi_RNN(object):
    def __init__(self, 
        ...
        self.DNN_settings = map(int, DNN_descriminator_setting.split('x'))

    def get_DNNscore_matrix(self, batch_rep1, batch_rep2):
        ...
        score_reshape_to_matrix = all_flat_score.reshape((self.batch_m, self.batch_m))
        return score_reshape_to_matrix
    ...
    def batch_DNNscore(self, batch_vectors1, batch_vectors2):
        all_input_pair = T.concatenate([batch_vectors1, batch_vectors2], axis = 1)
        all_flat_score = lasagne.layers.get_output(self.DNN.output, {self.DNN.l_in: all_input_pair})

        return all_flat_score
    ...
    def model_constructor(self, wemb_size = None):
        ...
        score_matrix = self.get_DNNscore_matrix(self.train_encodinglayer_vecs[0], self.train_encodinglayer_vecs[1]) 
        ...
        # test ending2
        self.score2 = self.batch_DNNscore(self.train_encodinglayer_vecs[0], self.train_encodinglayer_vecs[2])
        self.score1 = self.batch_DNNscore(self.train_encodinglayer_vecs[0], self.train_encodinglayer_vecs[1])
