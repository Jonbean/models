...
class Hierachi_RNN(object):
    ...
    def encoding_layer(self):
        ...
        end_representation = lasagne.layers.get_output(l_pooling, {l_end_in:ending_sequence_tensor})
    ...
    def attention_layer(self):        
        for i in range(self.story_nsent):
            ...
            broadcastable_zeros = T.zeros_like(self.train_encodinglayer_vecs[i])

            end_batch, end_feature = self.train_encodinglayer_vecs[4].shape
            broadcast_ending_tensor = self.train_encodinglayer_vecs[4].reshape([end_batch, 1, end_feature]) + broadcastable_zeros
            attention_score_matrix = theano.scan(lambda k: theano.scan(lambda j: T.dot(self.train_encodinglayer_vecs[i][k][j],broadcast_ending_tensor[k][j]), 
                                                                        sequences = [T.arange(n_seq)])[0], 
                                                                        sequences = [T.arange(n_batch)])[0]
            softmax_mask = T.switch(T.eq(self.inputs_masks[i], T.ones_like(self.inputs_masks[i])), self.inputs_masks[i], float('-inf'))

            attention_weight_matrix = T.nnet.softmax(attention_score_matrix*softmax_mask)
            attentioned_sent_rep = self.train_encodinglayer_vecs[i]*attention_weight_matrix.reshape([n_batch, n_seq, 1])
            self.attentioned_sent_rep.append(attentioned_sent_rep)
    ...
    def model_constructor(self, wemb_size = None):
        ...
        self.merge_ls = [T.reshape(tensor, (tensor.shape[0], 1, tensor.shape[1])) for tensor in self.attentioned_sent_rep]
